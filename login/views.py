...
from .models import Customer,BookManager
...
def manager_login(request):
    ...
    if request.method=="POST":
        ...
        if not len(username):
            ...
            pass
        if BookManager.objects.filter(username=username):
            user=BookManager.objects.filter(username=username)[0]
            password_hash=user.password
            res=check_password(password,password_hash)
            if res==1:
                request.session['username'] = username
...
def manager_signup(request):
    ...
    if request.method=="POST":
        username=request.POST['username']
        email=request.POST['email']
        if BookManager.objects.filter(username=username) or BookManager.objects.filter(email=email):
           messages.warning(request,"Account already exist, please Login to continue")
        else:
            password=request.POST['password']
            profile_pic=request.FILES.get('profile_pic',None)
            ...
            if(len(error)==0):
                password_hash = make_password(password)
                r_manager=BookManager(username=username,password=password_hash,email=email,phone_no=phone_no,profile_pic=profile_pic)
                r_manager.save()
                messages.info(request,"Account Created Successfully, Please login to continue")
